...
- UI: a Playwright smoke test (Chromium, desktop + 390px/360px touch viewports) driving rules/tips → practice (keyboard answers, feedback, show-answer, Ctrl+1 ignored) → test (clock, no feedback, report with percentile) → stats persistence + stored-XSS non-execution → tier-4 row locking/undo → double-tap answering exactly one question, double-Escape keeping the report, zero horizontal overflow, ≥44px touch targets.

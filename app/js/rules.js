...
export function rulesHtml() {
  ...
  const topL1 = row(['square', 'triangle', 'circle', 'cross']);
  const codeL1 = [3, 1, 4, 2];
...
<div class="tip-block">
  ...
  <p>One light row of three codes. Pick the one that turns the top row into the bottom row.</p>
  ...
  <ol class="slot-list">
    ...
    <li>Next bottom symbol ${chip(outL1[1])} → top position <strong>1</strong>. Write 1.</li>
    <li>${chip(outL1[2])} → position <strong>4</strong>. ${chip(outL1[3])} → position <strong>2</strong>.</li>
    <li>You wrote <strong>3-1-4-2</strong> → pick <strong>3142</strong> from the options. Done.</li>
  </ol>
</div>

<div class="tip-block rules-key">
  <h3>Level 2 — double switch, one given (read this twice)</h3>
  <p>Now there are <strong>two switches stacked</strong>. One is a <strong>dark card</strong>; one is a light row of three options.</p>
  <ul>
    <li>The dark card is <strong>not a choice</strong> — that switch is <em>always applied</em>, exactly as printed.</li>
    <li>The symbols pass through <strong>both</strong> switches in order: top row → first switch → a <strong>hidden middle row</strong> → second switch → bottom row.</li>
    ...
    <li>Your answer is always one of the three <strong>light</strong> cards.</li>
  </ul>

  <h4>Case A — dark card on top (work downwards)</h4>
  <p>Question: top ${chip(topA[0])}${chip(topA[1])}${chip(topA[2])}${chip(topA[3])}, dark card ${codeCard(givenA, true)} above the options, bottom ${chip(outA[0])}${chip(outA[1])}${chip(outA[2])}${chip(outA[3])}.</p>
  <ol class="slot-list">
    <li>Apply the dark code ${codeCard(givenA, true)} to the top row and <em>write the middle row down</em>:</li>
  </ol>
  ...
  <h4>Case B — dark card on the bottom (work upwards)</h4>
  <p>Question: top ${chip(topB[0])}${chip(topB[1])}${chip(topB[2])}${chip(topB[3])}, options first, then dark card ${codeCard(givenB, true)}, bottom ${chip(outB[0])}${chip(outB[1])}${chip(outB[2])}${chip(outB[3])}.</p>
  <ol class="slot-list">
    <li><strong>Undo</strong> the dark code to recover the middle row. Its digit <em>k</em> says where bottom slot <em>k</em> CAME FROM — so put the bottom's slot-<em>k</em> symbol back at middle position <em>digit k</em>. Dark code ${codeCard(givenB, true)}: put ${chip(outB[0])} at position 2, ${chip(outB[1])} at position 1, ${chip(outB[2])} at position 4, ${chip(outB[3])} at position 3:</li>
  </ol>
...
<div class="tip-block">
  <h3>Level 3 — triple switch, two dark cards</h3>
  <p>Same idea, three switches: top → switch 1 → middle row 1 → switch 2 → middle row 2 → switch 3 → bottom. Two are dark (always applied); you pick the third.</p>
  <ul>
    <li>Dark cards <em>above</em> the options: walk downwards through them, writing each middle row, then finish like Level 1.</li>
    <li>Dark cards <em>below</em>: undo them upwards from the bottom (Case B, twice).</li>
    <li>Options in the <em>middle</em> ("sandwich"): walk down through the first dark card AND up through the last one — the answer is the code linking the two middle rows you wrote.</li>
    <li>When two dark cards sit next to each other you can merge them into one code first — see the Tips page ("compose the two givens").</li>
  </ul>
</div>

<div class="tip-block">
  <h3>Level 4 — two switches, both unknown</h3>
  <p>Two light rows share the same three codes, and no dark card. Pick <strong>one code per row</strong> (first press picks row 1, second picks row 2 — Backspace un-picks). Exactly one combination works.</p>
...
<div class="tip-block">
  <h3>How the levels and the test work</h3>
  <ul>
    <li><strong>Practice</strong>: pick a fixed level to drill, or auto-ramp. Wrong answers show the full chain with every middle row.</li>
    <li><strong>Test</strong>: 6 minutes, as many questions as you can. Two right in a row moves you up a level; one wrong drops you down — exactly why accuracy beats speed.</li>
